    ...
    def get(self,request,formate=None):
        try:
            ...
            if not Listing.objects.filter(slug=slug,is_published=True).exists():
                return Response(
                {'error':'Published Listing with slug does not exists'},
                status=status.HTTP_404_NOT_FOUND
            )
            ...
            return Response(
                {'error':'Something went wrong when Retrieving Listings details'},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
...
class ListingsView(APIView):
    ...
    def get(self, request, format=None):
        try:
            # Fetch all listings, regardless of is_published status
...
class SearchListingsView(APIView):
    ...
    def get(self, request, format=None):
        try:
            ...
            # Ensure bathrooms is within a reasonable range
            if bathrooms < 0 or bathrooms >= 10:
                bathrooms = 1.0
            bathrooms = round(bathrooms, 1)

            # Sale type validation and normalization
            ...
            # Home type validation and normalization
            ...
            # Search query setup using PostgreSQL full-text search
            ...
            logging.error(f"Error when searching for listings: {str(e)}")
            return Response(
